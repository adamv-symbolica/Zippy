...
| Scalar units and fitted residuals are incomparable | `SpatialCostComponents` carries `nodeVisits`, `pathComparisons`, `allocations`, and `rounds`; all executors tick matching scoped counters. Fitted ns/unit and residual gates were removed. | Per-component prediction ≥ counter and ≤ 8× counter, plus an exact cross-executor three-round iteration; wall Spearman is smoke-only. |
