...
| `game-of-life` | `⌊result⌋=0, ⌈result⌉=min((min((min((min(liveCells, 127) * min(liveCells, 127, (min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127)), (min(liveCells, 127, (min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(liveCells, 127) + min(…, …))) + … + … + … + … + … + … + …))), …, …, …, …) * … * …), …, …, …) + … + … + … + … + … + … + …), …, …, …, …, …, …, …, …, …, …)` | `minPathLen≥3, maxPathLen≤3` | 8 |
...
| `scc-mutual-reachability` | `⌊result⌋=0, ⌈result⌉=(sccEdges * sccEdges)` | `minPathLen≥2, maxPathLen≤2` | 1 |
