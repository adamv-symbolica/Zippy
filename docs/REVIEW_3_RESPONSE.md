...
| One generic backend formula | `SpatialCostModel` has reference, trie, zipper, and graph implementations with operation-specific transfers. | Per-backend measured Spearman test plus “every backend wins” gate. |
...
| Monolithic implementation | Membership, shape, facts, costs, recursion, specialization, and law registry now live in separate modules. | Normal compilation boundary. |
...
| Analysis has no production consumer | `Supercompiler.specializeSpatially` invokes the guarded elimination/specialization pass. | Corpus equivalence and annotation-only tests. |
